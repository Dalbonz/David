...
- 한 일: 아버지(Codespaces)와 아들(채팅)이 각자 세션을 시작/종료할 때 GitHub 상태와 어긋나지
  ...
  설명은 `docs/PROCESS.md`에, Claude Code가 세션마다 자동으로 읽어 실제로 동작하게 만드는
  트리거는 리포 루트 `CLAUDE.md`에 작성. `docs/HANDOFF.md` 2번(파일 구조)에 두 파일을 반영하고,
  12번(개발 환경 분리 방식)에 시동어 절차를 요약해 추가.
- 변경한 파일: `docs/PROCESS.md`(신규), `CLAUDE.md`(신규, 리포 루트), `docs/HANDOFF.md`,
  `docs/PROGRESS.md`(이 기록)
- 확인: "다비드"가 이 Codespaces/Claude Code 세션에서 실제로 작동하는지는 다음 세션에서
  대화를 "다비드"로 시작해봐야 확인됨 — **확인 필요**. 채팅(Cowork) 쪽에 이 절차를 자동으로
  걸 방법이 있는지도 **확인 필요** (Cowork에 프로젝트 지침 저장 기능이 있는지 다음에 아들과
  함께 확인할 것).
- 결정 또는 위험: "다비드"라는 말 자체가 세션 간 기억을 자동으로 이어주는 건 아니라는 점을
  분명히 함 — 실제 연결고리는 여전히 GitHub 저장소(HANDOFF.md/PROGRESS.md/ROADMAP.md)이고,
  시동어는 그 절차를 사람이 잊지 않고 실행하게 만드는 신호일 뿐이다. 또한 이 "다비드"(세션
  시동어)는 나중에 만들 DAVID 앱 자체의 음성 웨이크워드(HANDOFF.md 4번)와 이름은 같지만
  완전히 다른 개념이라 `docs/PROCESS.md`에 명시적으로 구분해뒀다.
- 다음: 다음 세션을 "다비드"로 시작해서 실제로 절차가 지켜지는지 확인. Cowork 쪽 자동화 방법
  아들과 상의.
